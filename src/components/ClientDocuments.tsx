...
import * as apiStorage from '@/utils/apiStorage';
        ...
        {!caseNumber && (
          <div className="space-y-2">
            ...
            <div className="flex gap-2 items-center">
              <Select
                value={selectedCase || "no_case"}
                onValueChange={(v) => setSelectedCase(v === "no_case" ? "" : v)}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="General / no case..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="no_case">General (no case)</SelectItem>
                  {cases.map((caseItem, idx) => (
                    <SelectItem
                      key={caseItem.caseNumber || `idx-${idx}`}
                      value={caseItem.caseNumber || `unknown-case-${idx}`}
                    >
            ...
            <p className="text-xs text-muted-foreground">
              Optional — case uploads also live under each case&apos;s Case Documents accordion.
              This tab is for client-level misc files only.
            </p>
          </div>
        )}
